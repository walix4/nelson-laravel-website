...
const LOADING_STEPS = ["Locating the move…", "Pricing the payment…", "Calculating fees & escrow…", "Estimating net payout…"];
    ...
    <div className="max-w-[680px] mx-auto reveal">
      <div className="rounded-lg p-6 md:p-8 relative overflow-hidden shadow-2xl" style={{ background: "#fff", minHeight: phase === "form" ? undefined : 520 }}>
        <div className="flex items-center justify-between">
          <div><div className="text-[10px] uppercase tracking-[0.16em] font-bold text-[var(--navy)]/70">Instant payment engine</div><h3 className="display text-[26px] md:text-[28px] text-[var(--navy)] mt-1">Estimate a payment</h3></div>
        ...
        {phase !== "result" && (
          <form className="mt-6 space-y-3.5" onSubmit={(e) => { e.preventDefault(); run(); }}>
            <div><label className="input-label">Pickup <span className="text-[var(--red)]">*</span></label><input className="input mt-1" required value={from} onChange={(e) => setFrom(e.target.value)} placeholder="Pickup address or city" /></div>
            <div><label className="input-label">Delivery <span className="text-[var(--red)]">*</span></label><input className="input mt-1" required value={to} onChange={(e) => setTo(e.target.value)} placeholder="Delivery address or city" /></div>
            ...
            <button type="submit" className="btn-primary w-full py-4 rounded-lg text-[15px] font-semibold" style={{ marginTop: "1.5rem" }}><span className="label">Estimate payment</span></button>
            <p className="text-[10px] text-[var(--navy)]/55 text-center mt-1">No login · No card · Fees shown up front</p>
          </form>
        )}
        ...
        {phase === "result" && res && (
          <div className="mt-6">
            <div className="flex items-end justify-between">
              <div>
                <div className="text-[10px] uppercase tracking-[0.16em] font-bold text-[var(--green)] flex items-center gap-1.5"><span className="live-dot" /> Estimated payment</div>
                <div className="flex items-baseline gap-2 mt-1"><span className="display text-[48px] text-[var(--navy)] num leading-none">${N(res.total)}</span><span className="text-[13px] text-[var(--navy)]/60">/ move</span></div>
              </div>
              <div className="text-right text-[12px] text-[var(--navy)]/70"><div className="num"><b>{N(res.miles)}</b> mi · <b>{res.plazas}</b> milestones</div><div className="num"><b>{res.eta}</b> hr · <b>${res.perMile.toFixed(2)}</b>/mi</div></div>
            </div>
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-[10px]">
              {([["Linehaul", res.roads], ["Accessorials", res.bridges], ["Platform fee", res.oversize], ["Net payout", res.surcharge]] as [string, number][]).map(([k, v]) => <div key={k} className="rounded-lg px-3 py-2.5 bg-[var(--navy)]/5 border border-[var(--navy)]/10"><div className="text-[var(--navy)]/55 uppercase tracking-wider">{k}</div><div className="display text-[var(--navy)] text-[17px] num mt-0.5">${N(v)}</div></div>)}
            </div>
            <div className="mt-4 flex items-center gap-2.5">
              ...
              <button className="flex-1 py-3 rounded-lg text-[13px] font-semibold border border-[var(--navy)]/20 text-[var(--navy)] hover:bg-[var(--navy)]/5">Save payment</button>
            </div>
        ...
        {phase === "loading" && (
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center px-6" style={{ background: "linear-gradient(180deg,rgba(255,255,255,0.94),rgba(240,247,255,0.92))", backdropFilter: "blur(10px)" }}>
            ...
            <div className="display text-[16px] text-[var(--navy)] mt-5">Pricing your payment</div>
