    ...
    <section id="how" className="py-24" style={{ background: "linear-gradient(180deg,#FFFFFF,#F6F8FB)" }}>
      <div className="max-w-[1280px] mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 reveal">
          ...
          <Link href="/technology" className="shrink-0 rounded px-6 py-3 text-[14px] font-semibold text-[var(--navy)] border border-[var(--navy)]/15 hover:bg-[var(--navy)]/5 transition">See the technology</Link>
        </div>
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={s.n} className={`reveal reveal-d${i} rounded-lg p-7 transition ${hot ? "border-2" : "border"}`} style={{ background: hot ? "linear-gradient(170deg,#EFF4FC,#FFFFFF)" : "#fff", borderColor: hot ? "var(--red)" : "rgba(11,45,92,0.08)" }}>
                ...
                <Link href="/network" className="inline-flex items-center gap-1.5 mt-5 text-[13.5px] font-semibold text-[var(--red)]">Get started <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M9 6l6 6-6 6" /></svg></Link>
              </div>
            );
          })}
        </div>
