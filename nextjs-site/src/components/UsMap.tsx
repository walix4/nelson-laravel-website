...
const COLOR = { port: "#3A5FC0", origin: "#00a2e7", dest: "#16B571" };
    ...
    <div className="relative w-full overflow-hidden rounded-2xl" style={{ height, background: "linear-gradient(160deg,#0B2D5C,#061A38)" }}>
      ...
      {route && (
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
          <line x1={route[0].x} y1={route[0].y} x2={route[1].x} y2={route[1].y} stroke="#00a2e7" strokeWidth="0.5" strokeDasharray="2 2" opacity="0.8" />
        </svg>
      )}
      {dots.map((d, i) => (
