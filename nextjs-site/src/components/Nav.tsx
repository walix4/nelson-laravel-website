    ...
    <header className="sticky top-0 z-40 border-b" style={{ background: "#0B2D5C", borderColor: "rgba(255,255,255,0.18)" }}>
      <div className="max-w-[1400px] mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-5 text-[13px] font-medium text-white/85">
          <Link href="/tools" className="hover:text-white">Payment tools</Link>
          <Link href="/pricing" className="hover:text-white">How it works</Link>
          <div className={`mega-wrap ${open ? "open" : ""}`}>
            ...
            <div className="mega-panel">
              ...
              <div className="border-t mt-3 pt-4 flex flex-wrap gap-3" style={{ borderColor: "rgba(11,35,80,0.1)" }}>
                <Link href="/#quote" className="text-[13px] font-semibold px-5 py-2.5 rounded-lg" style={{ color: "var(--blue)", background: "rgba(58,95,192,0.08)" }}>Estimate a payment</Link>
        ...
        <div className="flex items-center gap-2.5">
          <Link href="/pricing" className="btn-primary text-[13px] px-4 py-2 rounded-lg inline-flex items-center gap-1.5"><span className="label">Open Wallet</span></Link>
        </div>
      </div>
    </header>
  );
