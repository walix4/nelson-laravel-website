...
export default function UnitConverter({ initial = "Length" }: { initial?: string }) {
  ...
  return (
    <div className="uc-card p-7 md:p-12">
      <div className="relative" style={{ zIndex: 1 }}>
        <div className="mb-6 md:max-w-[48%]">
          <select className="uc-select" value={cat} onChange={(e) => setCat(e.target.value)}>{Object.keys(UC).map((c) => <option key={c}>{c}</option>)}</select>
        </div>
        <div className="grid md:grid-cols-[1fr_auto_1fr] gap-5 md:gap-7 items-center">
          <div className="space-y-4">
            <select className="uc-select" value={from} onChange={(e) => setFrom(e.target.value)}>{units.map((u) => <option key={u}>{u}</option>)}</select>
            <input className="uc-num" inputMode="decimal" value={input} onChange={(e) => setInput(e.target.value)} />
          </div>
          <div className="uc-eq text-center select-none">=</div>
          <div className="space-y-4">
            <select className="uc-select" value={to} onChange={(e) => setTo(e.target.value)}>{units.map((u) => <option key={u}>{u}</option>)}</select>
            <input className="uc-num" readOnly value={out} style={{ background: "#F4F6FB" }} />
          </div>
        </div>
