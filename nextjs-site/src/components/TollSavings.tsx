...
export default function TollSavings() {
  ...
  return (
    <div className="reveal rounded-md bg-white/[0.08] border border-[#2f61c0]/30 backdrop-blur-sm shadow-2xl p-6 md:p-7 w-full max-w-[460px] mx-auto lg:mx-0 relative overflow-hidden">
      {phase !== "result" && (
        <form onSubmit={run} className="flex flex-col">
          ...
          <div className="mt-5 space-y-3.5">
            <div><label className={labelCls}>Select port terminal <span className="text-[#2f61c0]">*</span></label><PortSelect value={from} onChange={setFrom} options={PORTS} placeholder="Select port terminal" /></div>
            <div><label className={labelCls}>Enter drop off address <span className="text-[#2f61c0]">*</span></label><input className={fieldCls} required value={to} onChange={(e) => setTo(e.target.value)} placeholder="Enter drop-off address" /></div>
          </div>
      ...
      {phase === "loading" && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center px-6" style={{ background: "rgba(8,18,38,0.78)", backdropFilter: "blur(8px)" }}>
          <div className="relative w-16 h-16"><div className="absolute inset-0 rounded-full border-[3px] border-white/15" /><div className="absolute inset-0 rounded-full border-[3px] border-transparent border-t-[#2f61c0] animate-spin" /></div>
