...
function CounterStat({ val, suffix, prefix, label, decimals = 0 }: { val: number; suffix: string; prefix: string; label: string; decimals?: number }) {
  ...
  return (
    <div ref={ref} className="text-center">
      <div className="display text-[42px] md:text-[52px] text-[var(--navy)] num leading-none">
        {prefix}{count.toLocaleString()}{suffix}
      </div>
      <div className="mt-2 text-[13px] text-[var(--muted)] uppercase tracking-[0.12em] font-medium">{label}</div>
    </div>
  );
}
...
export default function Home() {
  return (
    <>
      {/* TICKER BAR */}
      <div className="text-[11px] font-medium border-b text-white" style={{ background: "#ffde01", borderColor: "rgba(255,255,255,0.2)" }}>
        <div className="max-w-[1400px] mx-auto px-6 h-8 flex items-center justify-between">
          <div className="flex items-center gap-5">
            ...
            <span className="hidden sm:inline opacity-60">·</span><span className="hidden sm:inline num"><b>50</b> state rule sets tracked</span>
            <span className="hidden md:inline opacity-60">·</span><span className="hidden md:inline num">Federal limit <b>80,000</b> lb · Tandem <b>34,000</b> lb</span>
          </div>
        ...
        <div className="max-w-[1400px] mx-auto px-6 relative">
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            ...
            <motion.div {...fadeUp} transition={{ duration: 0.55, delay: 0.1 }} className="rounded-2xl p-8" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
              ...
              {[
                ["Hours", "Manual permit research per load"],
                ["$10k+", "Average overweight fine exposure"],
                ["3–5 days", "Multi-state permit turnaround"],
                ["Unknown", "Real-time axle compliance status"],
                ["Missed", "Daily state rule-set changes"],
              ].map(([val, label], i) => (
            ...
            <motion.div {...fadeUp} transition={{ duration: 0.55, delay: 0.2 }} className="rounded-2xl p-8" style={{ background: "rgba(255,222,1,0.06)", border: "1px solid rgba(255,222,1,0.2)" }}>
              <div className="text-[13px] font-bold uppercase tracking-[0.14em] mb-6 flex items-center gap-2" style={{ color: "#ffde01" }}>
                <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#ffde01", display: "inline-block" }} />
                With DrayOW
              </div>
              {[
                ["Seconds", "Instant overweight compliance check"],
                ["$0", "Fine exposure with permit coverage"],
                ["Instant", "Multi-state permit generation"],
                ["Live", "Real-time axle status every load"],
                ["Automatic", "Daily rule-set sync, all 50 states"],
              ].map(([val, label], i) => (
      ...
      {/* ═══ FEATURE GRID ═══ */}
      <section id="features" className="py-24 relative overflow-hidden" style={{ background: "linear-gradient(180deg,#F8FAFC,#EEF2F8)" }}>
        <div className="absolute inset-0 pointer-events-none opacity-40" style={{ background: "radial-gradient(900px 600px at 80% 20%,rgba(58,95,192,0.14),transparent 60%)" }} />
        <div className="max-w-[1400px] mx-auto px-6 relative">
          <motion.div {...fadeUp} className="max-w-2xl mb-14">
            <div className="text-[11px] uppercase tracking-[0.22em] font-semibold mb-3" style={{ color: "var(--red)" }}>Platform capabilities</div>
            <h2 className="display text-[36px] md:text-[50px] text-[var(--navy)] leading-[1.04]">Everything overweight compliance requires.</h2>
            <p className="mt-4 text-[var(--muted)] text-[15px] max-w-xl">Six core capabilities, fully automated. From the moment a load is submitted to the moment it crosses the state line legally.</p>
          </motion.div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {FEATURES.map((f, i) => (
              <motion.div
                ...
                style={{ border: "1px solid rgba(11,45,92,0.08)", transition: "border-color 0.2s" }}
              >
                <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-5" style={{ background: f.accent === "#ffde01" ? "rgba(255,222,1,0.12)" : "rgba(58,95,192,0.1)" }}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={f.accent} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    <path d={f.icon} />
                  </svg>
                </div>
                <h3 className="display text-[18px] text-[var(--navy)] mb-2">{f.title}</h3>
      ...
      <section id="how-it-works" className="py-24 relative overflow-hidden bg-white">
        <div className="max-w-[1400px] mx-auto px-6">
          <motion.div {...fadeUp} className="text-center max-w-2xl mx-auto mb-16">
            <div className="text-[11px] uppercase tracking-[0.22em] font-semibold mb-3" style={{ color: "var(--red)" }}>How it works</div>
            <h2 className="display text-[36px] md:text-[50px] text-[var(--navy)] leading-[1.04]">From submission to compliant move in minutes.</h2>
          </motion.div>
          <div className="relative">
            {/* connecting line */}
            <div className="hidden lg:block absolute top-[52px] left-[10%] right-[10%] h-px" style={{ background: "linear-gradient(90deg,transparent,rgba(11,45,92,0.15) 20%,rgba(11,45,92,0.15) 80%,transparent)" }} />
              ...
              {WORKFLOW_STEPS.map((s, i) => (
                ...
                >
                  <div className="relative mb-5">
                    <div className="w-[52px] h-[52px] rounded-full flex items-center justify-center display text-[15px] text-[var(--navy)] bg-white" style={{ border: `2px solid ${s.color}`, position: "relative", zIndex: 2 }}>{s.n}</div>
                    {i < WORKFLOW_STEPS.length - 1 && (
                      <div className="lg:hidden absolute top-1/2 left-full w-8 h-px ml-2" style={{ background: "rgba(11,45,92,0.15)" }} />
                    )}
                  </div>
                  <h3 className="display text-[17px] text-[var(--navy)] mb-2">{s.title}</h3>
                  <p className="text-[13px] text-[var(--muted)] leading-relaxed max-w-[180px]">{s.desc}</p>
                </motion.div>
              ))}
            </div>
          </div>
      ...
      {/* ═══ LIVE LOADS IN PROCESS ═══ */}
      <section className="py-24 relative overflow-hidden" style={{ background: "linear-gradient(180deg,#F8FAFC,#EEF4F9)" }}>
        <div className="max-w-[1400px] mx-auto px-6 relative">
          <motion.div {...fadeUp} className="text-center max-w-2xl mx-auto mb-12">
            <div className="text-[11px] uppercase tracking-[0.22em] font-semibold mb-2" style={{ color: "var(--red)" }}>Live on the network</div>
            <h2 className="display text-[36px] md:text-[48px] text-[var(--navy)] leading-[1.04]">Loads being checked right now</h2>
            <p className="mt-4 text-[var(--muted)] text-[15px]">Real overweight checks running across the country — by axle configuration, from 5-axle semis to oversize permit loads.</p>
            <div className="mt-6 flex items-center justify-center gap-4">
              <Link href="/estimates" className="text-[14px] font-semibold text-[var(--navy)] inline-flex items-center gap-1.5">View all <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="text-[var(--navy)]/35"><path d="M5 12h14M13 6l6 6-6 6"/></svg></Link>
              <Link href="/#quote" className="px-5 py-2.5 rounded-lg text-[13px] font-semibold bg-[var(--navy)] text-white hover:bg-[var(--navy-2)] transition">Check a load</Link>
            </div>
          </motion.div>
          <div className="grid md:grid-cols-3 gap-6">
            {SHIP.map((s, i) => (
              <motion.div key={s.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.5 }} className="bg-white rounded-2xl p-6" style={{ border: "1px solid rgba(11,45,92,0.08)" }}>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2.5">{s.icon}<h3 className="display text-[17px] text-[var(--navy)]">{s.title}</h3></div>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--green)" strokeWidth="1.8"><circle cx="12" cy="12" r="9"/><path d="M12 11v5" strokeLinecap="round"/><circle cx="12" cy="7.8" r="0.6" fill="var(--green)"/></svg>
                </div>
        ...
        <div className="max-w-[1400px] mx-auto px-6 relative">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <motion.div {...fadeIn} transition={{ duration: 0.7, delay: 0.2 }} className="relative" style={{ minHeight: 400 }}>
                  ...
                  {[["Route: LAX → PHX","5-axle","Legal","#22c55e"],["Route: LGB → LV","6-axle","Permit","#ffde01"],["Route: OAK → SAC","5-axle","Legal","#22c55e"],["Route: HOU → DAL","6-axle","Escort","#E53935"]].map(([r,ax,s,c]) => (
                    <div key={r} className="rounded-xl p-3 flex items-center justify-between" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.06)" }}>
                      <div>
                        ...
                        <div className="text-white/40 text-[10px] mt-0.5">{ax}</div>
                      </div>
              ...
              {/* floating tags */}
              {[
                { label: "Axle config", val: "5-axle semi", style: { top: "-14px", left: "10%", animationDelay: "-1s" } },
                { label: "Gross weight", val: "38,420 lb",  style: { top: "30%",   right: "-10px", animationDelay: "-2.5s" } },
                { label: "Status",       val: "Legal",      style: { bottom: "20%",left: "-8px",   animationDelay: "-3.5s" } },
              ].map(t => (
      ...
      {/* ═══ ANIMATED STATS ═══ */}
      <section className="py-20 bg-white" style={{ borderTop: "1px solid rgba(11,45,92,0.06)", borderBottom: "1px solid rgba(11,45,92,0.06)" }}>
        <div className="max-w-[1400px] mx-auto px-6">
      ...
      <section className="py-24 relative overflow-hidden" style={{ background: "linear-gradient(180deg,#F8FAFC,#EEF2F8)" }}>
        <div className="max-w-[1100px] mx-auto px-6">
          <motion.div {...fadeUp} className="text-center max-w-2xl mx-auto mb-14">
            <div className="text-[11px] uppercase tracking-[0.22em] font-semibold mb-3" style={{ color: "var(--red)" }}>Why DrayOW</div>
            <h2 className="display text-[36px] md:text-[50px] text-[var(--navy)] leading-[1.04]">The only platform built end-to-end for overweight compliance.</h2>
          </motion.div>
          <motion.div {...fadeIn} transition={{ duration: 0.5, delay: 0.15 }} className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(11,45,92,0.1)" }}>
            {/* header row */}
            <div className="grid grid-cols-4 text-[12px] font-bold uppercase tracking-[0.12em]" style={{ background: "var(--navy)", color: "#fff" }}>
              <div className="px-6 py-4 col-span-1">Feature</div>
              <div className="px-6 py-4 text-center" style={{ background: "rgba(255,222,1,0.15)", color: "#ffde01" }}>DrayOW ✦</div>
              <div className="px-6 py-4 text-center text-white/50">Manual</div>
              <div className="px-6 py-4 text-center text-white/50">Others</div>
            </div>
            {COMPARE_ROWS.map((r, i) => (
              <div key={r.feat} className="grid grid-cols-4 text-[13px]" style={{ background: i % 2 === 0 ? "#fff" : "#F8FAFC", borderTop: "1px solid rgba(11,45,92,0.06)" }}>
                <div className="px-6 py-4 text-[var(--navy)] font-medium">{r.feat}</div>
                <div className="px-6 py-4 text-center" style={{ background: "rgba(255,222,1,0.05)" }}><Check ok={r.us} /></div>
