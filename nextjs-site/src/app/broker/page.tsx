...
═══════════════════════════════════════════════════════════════════════════ */
const BROKER_LINES = [
  { text: "Match Faster. Move More.",   color: "#00a5e7" },
  { text: "Settle in 24 Hours.",        color: "#4ADE80" },
  { text: "1,800+ Vetted Carriers.",    color: "#fc0b05" },
];
...
export default function BrokerPage() {
  ...
  const [brokerOut, setBrokerOut] = useState(false);
  useEffect(() => {
    const t = setInterval(() => {
      setBrokerOut(true);
      setTimeout(() => { setBrokerIdx(i => (i + 1) % BROKER_LINES.length); setBrokerOut(false); }, 380);
    }, 2800);
  ...
  return (
    <div style={{ background: "#060f1e" }}>
      ...
      <section style={{ position:"relative", overflow:"hidden", borderBottom:"1px solid rgba(255,255,255,0.12)" }}>
        <picture>
          <source srcSet={asset("/broker-hero-bg.webp")} type="image/webp" />
          <img src={asset("/broker-hero-bg.jpg")} alt="" aria-hidden="true" style={{ position:"absolute", inset:0, width:"100%", height:"100%", objectFit:"cover", objectPosition:"center" }} />
        </picture>
        ...
        <div className="max-w-[1200px] mx-auto grid lg:grid-cols-[1.05fr_1fr] gap-10 items-center" style={{ padding:"clamp(56px,8vw,110px) clamp(24px,4vw,56px) 0", position:"relative", zIndex:1 }}>
          ...
          <div style={{ display:"inline-flex", alignItems:"center", gap:8, borderRadius:12, padding:"7px 16px", fontSize:11.5, fontWeight:800, letterSpacing:"0.14em", textTransform:"uppercase", color:"#00a5e7", background:"rgba(0,165,231,0.08)", border:"1px solid rgba(0,165,231,0.3)", width:"fit-content" }}>
            <span style={{ width:6, height:6, borderRadius:2, background:"#00a5e7", display:"inline-block" }} />
          ...
          <div style={{ height:"clamp(36px,3.5vw,48px)", display:"flex", alignItems:"center" }}>
            <span style={{ fontSize:"clamp(20px,2.3vw,32px)", fontWeight:900, letterSpacing:"-0.02em", color:BROKER_LINES[brokerIdx].color, whiteSpace:"nowrap", transition:"color 0.3s ease", animation:brokerOut ? "heroLineOut 0.38s ease forwards" : "heroLineIn 0.42s ease forwards", textShadow:`0 0 34px ${BROKER_LINES[brokerIdx].color}55` }}>
              {BROKER_LINES[brokerIdx].text}
            </span>
          </div>
          <p style={{ fontSize:"clamp(15px,1.25vw,17.5px)", color:"rgba(255,255,255,0.52)", lineHeight:1.75, maxWidth:480, margin:"14px 0 36px" }}>
            Connect to 1,800+ vetted drayage carriers across every major US port. Real rates, real capacity &mdash; real-time.
          </p>
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2" style={{ marginTop:28, fontSize:13, color:"rgba(255,255,255,0.45)" }}>
            <span style={{ display:"inline-flex", alignItems:"center", gap:7 }}>
              <span style={{ color:"#00a5e7", fontSize:15 }}>★★★★★</span> <b style={{ color:"#fff" }}>4.9</b> rating
            </span>
            <span><b style={{ color:"#fff" }}>1,800+</b> vetted carriers</span>
          ...
          <div className="reveal-right reveal-delay-1 flex" style={{ alignItems:"flex-end", justifyContent:"center", position:"relative", paddingTop:24 }}>
            <div style={{ position:"absolute", inset:0, background:"radial-gradient(ellipse 55% 55% at 50% 60%, rgba(0,165,231,0.18) 0%, transparent 70%)", pointerEvents:"none" }} />
      ...
      {/* ── BROKER CTA ──────────────────────────────────────────────────── */}
      <section style={{ position:"relative", background:"linear-gradient(160deg,#06101e 0%,#030a1c 55%,#06101e 100%)", padding:"100px 24px", overflow:"hidden" }}>
        <div style={{ position:"absolute", inset:0, background:"radial-gradient(ellipse 70% 70% at 50% 50%, rgba(0,165,231,0.10) 0%, transparent 65%)", zIndex:1 }} />
        ...
        <div style={{ position:"relative", zIndex:3, maxWidth:760, margin:"0 auto", textAlign:"center" }}>
          <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(0,165,231,0.14)", border:"1px solid rgba(0,165,231,0.40)", borderRadius:6, padding:"5px 14px", fontSize:10, fontWeight:800, letterSpacing:"0.16em", textTransform:"uppercase" as const, color:"#00a5e7", marginBottom:28 }}>
            <span style={{ width:5, height:5, borderRadius:"50%", background:"#00a5e7", display:"inline-block" }} />
            For Freight Brokers
          </div>
          <h2 style={{ fontSize:"clamp(34px,5vw,64px)", fontWeight:900, color:"#fff", lineHeight:1.05, margin:"0 0 18px", letterSpacing:"-0.02em" }}>
            Build your brokerage<br /><span style={{ color:"#00a5e7" }}>on DrayGo.</span>
          </h2>
          <p style={{ fontSize:18, color:"rgba(255,255,255,0.50)", lineHeight:1.65, margin:"0 auto 40px", maxWidth:520 }}>
            1,800+ vetted carriers, live rates, instant settlements. Everything your drayage brokerage needs in one dashboard.
          </p>
          <div style={{ display:"flex", gap:14, justifyContent:"center", flexWrap:"wrap", marginBottom:28 }}>
            <a href="#" style={{ display:"inline-flex", alignItems:"center", gap:8, background:"#00a5e7", color:"#fff", fontWeight:700, fontSize:16, borderRadius:10, padding:"18px 36px", textDecoration:"none", boxShadow:"0 8px 32px rgba(0,165,231,0.30)" }}>
              Start Free Trial
          ...
          <div style={{ display:"flex", gap:24, justifyContent:"center", flexWrap:"wrap" }}>
            {["No setup fees", "Live carrier network", "Cancel anytime"].map(t => (
              <div key={t} style={{ display:"flex", alignItems:"center", gap:7, fontSize:13, color:"rgba(255,255,255,0.40)" }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#00a5e7" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                {t}
              </div>
            ))}
          </div>
