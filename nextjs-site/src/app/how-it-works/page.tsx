    ...
    <>
      ...
      <section style={{ background: "#08192b", padding: "80px 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 32 }}>
          {STEPS.map((step, i) => (
            <div key={step.num} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0, background: "rgba(255,255,255,0.03)", border: `1px solid rgba(${step.rgb},0.18)`, borderRadius: 20, overflow: "hidden" }} className="how-step-card">
              {/* left: number + content */}
              <div style={{ padding: "52px 52px", borderRight: `1px solid rgba(${step.rgb},0.12)`, position: "relative" }}>
                <div style={{ position: "absolute", left: 52, top: 52, fontSize: 100, fontWeight: 900, color: `rgba(${step.rgb},0.07)`, lineHeight: 1, userSelect: "none" }}>{step.num}</div>
                <div style={{ position: "relative", zIndex: 1 }}>
                  <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: `rgba(${step.rgb},0.12)`, border: `1px solid rgba(${step.rgb},0.3)`, borderRadius: 6, padding: "4px 12px", fontSize: 10, fontWeight: 800, letterSpacing: "0.14em", textTransform: "uppercase", color: step.color, marginBottom: 20 }}>
              ...
              <div style={{ padding: "52px 52px", display: "flex", flexDirection: "column", justifyContent: "center", gap: 18 }}>
                ...
                {i < STEPS.length - 1 && (
                  <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{ fontSize: 11, color: `rgba(${step.rgb},0.7)`, fontWeight: 700, letterSpacing: "0.1em" }}>feeds into {STEPS[i + 1].tag} →</span>
                  </div>
                )}
              </div>
            </div>
      ...
      {/* ── WHY DIFFERENT ─────────────────────────────────────────────── */}
      <section style={{ background: "#060d1a", padding: "88px 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            ...
            <h2 style={{ fontSize: "clamp(28px,4vw,48px)", fontWeight: 900, color: "#fff", lineHeight: 1.05, margin: "0 0 16px" }}>
              The gap is where drayage friction lives.
            </h2>
            <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 16, maxWidth: 560, margin: "0 auto" }}>
              Most TMS platforms stop at visibility, then hand off to a completely separate, disconnected stack for money. That's where 45-day cycles, disputed invoices, and lost paperwork come from.
            </p>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }} className="diff-grid">
            {DIFFS.map(col => (
              <div key={col.label} style={{
                background: col.bad ? "rgba(255,255,255,0.02)" : "rgba(252,11,5,0.04)",
                border: col.bad ? "1px solid rgba(255,255,255,0.07)" : "1.5px solid rgba(252,11,5,0.25)",
                borderRadius: 18,
                padding: "36px 36px",
              }}>
                <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", color: col.bad ? "rgba(255,255,255,0.35)" : "#fc0b05", marginBottom: 24 }}>{col.label}</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  {col.items.map(item => (
                    <div key={item} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <div style={{ width: 22, height: 22, borderRadius: 6, background: col.bad ? "rgba(255,255,255,0.05)" : "rgba(252,11,5,0.12)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        {col.bad
                          ? <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="2.5" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                      ...
                      <span style={{ fontSize: 14, color: col.bad ? "rgba(255,255,255,0.45)" : "rgba(255,255,255,0.85)", lineHeight: 1.4 }}>{item}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {/* closing statement */}
          <div style={{ marginTop: 48, background: "rgba(99,91,255,0.06)", border: "1px solid rgba(99,91,255,0.2)", borderRadius: 16, padding: "32px 40px", display: "grid", gridTemplateColumns: "1fr auto", alignItems: "center", gap: 32 }} className="closing-cta">
            <div>
              <div style={{ fontSize: 18, fontWeight: 700, color: "#fff", marginBottom: 8 }}>The hard-to-copy part</div>
              <p style={{ fontSize: 14.5, color: "rgba(255,255,255,0.55)", margin: 0, lineHeight: 1.65 }}>
            ...
            <a href="/#load-board" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#fc0b05", color: "#fff", fontWeight: 700, fontSize: 14, borderRadius: 8, padding: "14px 24px", textDecoration: "none", whiteSpace: "nowrap", flexShrink: 0 }}>
              Get Started
