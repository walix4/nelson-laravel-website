            ...
            <tbody>
              {txns.length > 0 ? txns.map(tx => (
                <tr key={tx.hash}>
                  ...
                  <td>
                    <span style={{ fontSize: 11, fontWeight: 600, padding: "2px 7px", borderRadius: 4, background: `${METHOD_COLORS[tx.method]}18`, color: METHOD_COLORS[tx.method], border: `1px solid ${METHOD_COLORS[tx.method]}30` }}>{tx.method}</span>
                  </td>
                  <td><Link href={`/block/${tx.block}`}>{tx.block.toLocaleString()}</Link></td>
                  <td style={{ color: "#64748b" }}>{tx.age}</td>
                  <td className="mono">
                    <span style={{ background: tx.from === addr ? "rgba(59,130,246,0.1)" : "transparent", borderRadius: 3, padding: "1px 4px" }}>
                      <Link href={`/address/${tx.from}`}>{addrLabel(tx.from)}</Link>
                    </span>
                  </td>
                  <td>
                    <span style={{ background: tx.to === addr ? "rgba(34,197,94,0.12)" : "rgba(59,130,246,0.12)", color: tx.to === addr ? "#22c55e" : "#3b82f6", border: `1px solid ${tx.to === addr ? "#22c55e" : "#3b82f6"}30`, borderRadius: 3, fontSize: 9, fontWeight: 700, padding: "1px 5px" }}>{tx.to === addr ? "IN" : "OUT"}</span>
                  </td>
