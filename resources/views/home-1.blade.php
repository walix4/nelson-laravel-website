    ...
    <div class="mx-auto max-w-7xl px-5 sm:px-8 pt-10 sm:pt-14 lg:pt-20 pb-16 sm:pb-20 lg:pb-28 grid lg:grid-cols-12 gap-10 lg:gap-8 items-center">
        ...
        <div class="order-2 lg:order-1 lg:col-span-7 relative">
            <div class="reveal reveal-left relative w-full max-w-[560px] sm:max-w-[680px] lg:max-w-none mx-auto lg:mx-0 lg:-ml-12 xl:-ml-20">
                ...
                <div class="reveal reveal-delay-2 absolute z-20 inline-flex items-center gap-2 rounded-full bg-red-600 text-white px-3 py-1.5 shadow-lg shadow-red-600/40 sos-glow" style="top:5%; right:14%;">
        ...
        <div class="mx-auto max-w-7xl px-5 sm:px-8 py-16 lg:py-24">
            ...
            <div class="reveal stagger mt-12 grid md:grid-cols-3 gap-6">
                @foreach ([
                    ...
                    ['n'=>'03','t'=>'Verified responder arrives','b'=>'The closest Super Agent is auto-dispatched. Watch their ETA, distance and live route on your map until they reach you.','c'=>'bg-emerald-100 text-emerald-600'],
                ] as $s)
                    <div class="relative rounded-md border border-ink-100 bg-white p-7 shadow-sm hover:shadow-lg transition">
                        <span class="grid place-items-center w-12 h-12 rounded-md font-display text-xl font-bold {{ $s['c'] }}">{{ $s['n'] }}</span>
                        <h3 class="mt-5 text-xl font-bold text-navy-900">{{ $s['t'] }}</h3>
                        <p class="mt-2 text-sm text-navy-700/80 leading-relaxed">{{ $s['b'] }}</p>
                    </div>
                @endforeach
            </div>
            ...
            <div class="mt-16 grid lg:grid-cols-2 gap-8 items-center">
                <div class="reveal reveal-left">
                    ...
                    <p class="mt-4 text-navy-700/80 leading-relaxed">
                        Every report is signed by a real, vetted Super Agent. Evidence is encrypted on-device before upload. Your location is shared only with the agent who accepts your case — and revoked the second the case closes.
                    </p>
                    <ul class="mt-6 space-y-3 text-sm">
                        @foreach (['Background-checked Super Agents on every dispatch','End-to-end encryption on photos, audio, and video','Auto-shred of location data after case closure','Audit log on every status change'] as $li)
                            <li class="flex items-start gap-3">
                                <span class="mt-0.5 grid place-items-center w-5 h-5 rounded-full bg-emerald-100 text-emerald-600">
                                    <svg class="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                                </span>
                                <span class="text-navy-800">{{ $li }}</span>
                            </li>
                        @endforeach
                    </ul>
                </div>
                <div class="reveal reveal-right relative rounded-md bg-navy-950 text-white p-8 lg:p-10 overflow-hidden">
                    ...
                    <p class="relative mt-1 text-navy-300 text-sm">Newark, NJ · Sexual Crime · Tier 2</p>
                    <div class="relative mt-6 space-y-3 text-sm">
                        @foreach ([['t'=>'09:14:02','e'=>'Report submitted by victim','d'=>'Photos × 4, audio × 1'],['t'=>'09:14:08','e'=>'Auto-dispatch · 3 agents pinged','d'=>'Closest match: Agent J. Holland — 0.7 mi'],['t'=>'09:14:53','e'=>'Agent accepted — en route','d'=>'ETA 4 min'],['t'=>'09:18:11','e'=>'On scene','d'=>'Status: Active assist']] as $row)
                            <div class="flex items-start gap-3 border-l-2 border-brand-500/40 pl-3">
                                <span class="font-mono text-[11px] text-brand-300 shrink-0">{{ $row['t'] }}</span>
                                <div>
                                    <p class="font-semibold">{{ $row['e'] }}</p>
                                    <p class="text-navy-300 text-xs">{{ $row['d'] }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
