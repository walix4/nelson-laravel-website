...
<head>
    <meta charset="UTF-8" />
    <script>
      // Per-country lang attribute + auto-translate to Spanish on .mx/.do/.ec/.pr/.sv
      (function(){
        var h = location.hostname;
        var langMap = {
          'auxilio.mx': 'es-MX', 'www.auxilio.mx': 'es-MX',
          'auxilio.do': 'es-DO', 'www.auxilio.do': 'es-DO',
          'auxilio.ec': 'es-EC', 'www.auxilio.ec': 'es-EC',
          'auxilio.pr': 'es-PR', 'www.auxilio.pr': 'es-PR',
          'auxilio.sv': 'es-SV', 'www.auxilio.sv': 'es-SV',
          'auxilio.ca': 'en-CA', 'www.auxilio.ca': 'en-CA'
        };
        var lang = langMap[h] || 'en';
        document.documentElement.lang = lang;

        // Apply our own short Spanish nav labels (run as soon as DOM is parseable),
        // then mark them notranslate so Google Translate leaves them alone.
        function applyI18n(){
          var es = lang.indexOf('es') === 0;
          var attr = es ? 'data-i18n-es' : 'data-i18n-en';
        ...
        } else { applyCountryPages(); }

        // Auto-translate via Google Translate for Spanish-locale domains
        if (lang.indexOf('es') === 0) {
          // Set the googtrans cookie BEFORE the widget loads so it picks up the target language
          var domain = h.replace(/^www\./,'');
          document.cookie = 'googtrans=/en/es;path=/;domain=.' + domain;
          document.cookie = 'googtrans=/en/es;path=/';

          // Initialize widget (hidden — banner CSS-hidden below)
    ...
    <style>
      /* Hide Google Translate banner/iframe/skiptranslate body offset on auto-translate domains */
      #auxilio-gt-host { display: none !important; }
...
<script>
/* Hide all content until router runs — prevents home-hero flash on deep-link reload */
(function(){
    ...
    /* If visiting a non-home route, hide body until JS applies the correct view */
    ...
    if(allowed.indexOf(h)!==-1){
        ...
        window.addEventListener('DOMContentLoaded',function(){
            setTimeout(function(){ document.documentElement.style.visibility=''; },0);
        });
    }
})();
</script>
<body class="font-sans">
<div id="auxilio-gt-host" aria-hidden="true"></div>
...
<header data-nav style="background-color:#0a1a4a" class="fixed inset-x-0 top-0 z-50 text-white">
    <nav class="mx-auto flex h-[78px] max-w-7xl items-center px-5 sm:px-8">
        ...
        {{-- Logo — fixed width, never shrinks; brand text protected from translation --}}
        <a href="#top" class="flex shrink-0 items-center gap-2.5 group mr-8 notranslate" translate="no">
        ...
        <div class="ml-auto lg:ml-6 flex items-center gap-2.5 lg:border-l lg:border-white/20 lg:pl-6">
            <a href="https://auxilionetwork.com/siteadmin/login"    target="_blank" rel="noopener" class="inline-flex items-center justify-center h-10 px-5 text-sm font-bold rounded-lg transition hover:bg-white/90" style="background:#ffffff; color:#9b1c1c;" data-i18n-en="Login" data-i18n-es="Entrar">Login</a>
            <a href="https://auxilionetwork.com/siteadmin/register" target="_blank" rel="noopener" class="inline-flex items-center justify-center gap-1.5 h-10 px-5 text-sm font-bold text-white rounded-lg transition hover:opacity-90" style="background:#9b1c1c;"><span data-i18n-en="Get started" data-i18n-es="Empezar">Get started</span> <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg></a>
        </div>
    </nav>

    <div id="mobile-menu" data-menu-panel class="lg:hidden absolute inset-x-4 top-[72px] rounded-md bg-white border border-ink-100 shadow-xl p-6">
    ...
    <div class="relative mx-auto max-w-7xl px-5 sm:px-8 grid lg:grid-cols-12 gap-10 lg:gap-14 items-center" style="padding-top:2rem;padding-bottom:3rem;">
        ...
        <div class="lg:col-span-6">
            ...
            <h1 class="reveal reveal-delay-1 mt-5 font-display font-extrabold tracking-tight text-white text-[44px] sm:text-6xl lg:text-[72px] leading-[1.02]">
                Your safety,<br/>
                <span style="background:linear-gradient(90deg,#38bdf8 0%,#818cf8 55%,#a5b4fc 100%); -webkit-background-clip:text; background-clip:text; color:transparent;">your control.</span>
            </h1>
            ...
            {{-- App Store + Google Play download badges (same as agent page) --}}
            <div class="reveal reveal-delay-3 mt-8 flex flex-wrap items-center gap-3">
                <a href="#download" class="inline-flex shrink-0 transition hover:-translate-y-0.5 hover:opacity-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-500 rounded-lg"><img src="/images/app-store-badge.png" alt="Download on the App Store" class="h-12 sm:h-14 w-auto rounded-lg shadow-md" /></a>
                <a href="#download" class="inline-flex shrink-0 transition hover:-translate-y-0.5 hover:opacity-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-500 rounded-lg"><img src="/images/google-play-badge.png" alt="Get it on Google Play" class="h-12 sm:h-14 w-auto rounded-lg shadow-md" /></a>
            </div>
        ...
        <div class="lg:col-span-6 relative">
            <div class="reveal reveal-right relative mx-auto max-w-xl">
                ...
                <div class="relative rounded-[32px] overflow-hidden aspect-[4/5] ring-1 ring-white/15 hero-slideshow" style="box-shadow: 0 40px 90px -20px rgba(8,15,40,.7);">
                    <img src="/images/hero-slide-4.jpg" data-mx-src="/images/mx/hero-1.jpg?v=2" alt="Auxilio partner officers" class="hero-slide absolute inset-0 w-full h-full object-cover" />
                    <img src="/images/hero-slide-3.jpg" data-mx-src="/images/mx/hero-2.jpg?v=3" alt="Auxilio partner officers" class="hero-slide absolute inset-0 w-full h-full object-cover" />
                    <img src="/images/hero-slide-2.jpg" data-mx-src="/images/mx/hero-3.jpg?v=2" alt="Auxilio partner officers" class="hero-slide absolute inset-0 w-full h-full object-cover" />
                ...
                <style>
                    /* 3 slides × ~6.7s each in a 20s cycle (default — auxilio.app / auxilio.ca) */
                    .hero-slideshow .hero-slide { position:absolute; inset:0; width:100%; height:100%; object-fit:cover; opacity:0; animation: heroSlide 20s infinite; will-change: opacity; }
                    .hero-slideshow .hero-slide:nth-child(1) { animation-delay: 0s; opacity: 1; }
                    .hero-slideshow .hero-slide:nth-child(2) { animation-delay: 6.7s; }
                    .hero-slideshow .hero-slide:nth-child(3) { animation-delay: 13.3s; }
                    @keyframes heroSlide {
                        0%, 30%   { opacity: 1; }
                        35%, 98%  { opacity: 0; }
                        100%      { opacity: 1; }
                    }
                    .hero-slideshow .hero-dot { transition: all .3s ease; animation: heroDot 20s infinite; }
                    .hero-slideshow .hero-dot:nth-child(1) { animation-delay: 0s; }
                    .hero-slideshow .hero-dot:nth-child(2) { animation-delay: 6.7s; }
                    .hero-slideshow .hero-dot:nth-child(3) { animation-delay: 13.3s; }
                    @keyframes heroDot {
...
========================================================================--}}
<section class="relative pt-6 pb-10 overflow-hidden" style="background:#0a1a4a;">
    ...
    <div class="relative mx-auto max-w-5xl px-5 sm:px-8">
        ...
        <div class="reveal reveal-delay-3 mt-10 flex flex-wrap items-center justify-center gap-2" data-faq-filter>
            @foreach ($faqCats as $i => $c)
                <button type="button" data-faq-cat="{{ $c }}" class="faq-chip inline-flex items-center gap-1.5 rounded-md border border-ink-200 bg-white px-4 py-1.5 text-xs font-semibold uppercase tracking-[.14em] text-navy-900/70 hover:border-brand-300 hover:text-brand-600 transition {{ $i===0 ? 'is-active' : '' }}">
                    {{ $c }}
                </button>
            @endforeach
        </div>
...
<div id="cg-legend-bar" class="sticky top-[78px] z-30 border-b border-slate-100 shadow-sm" style="background:#fff;backdrop-filter:blur(12px);display:none">
    <div class="mx-auto max-w-7xl px-5 sm:px-8 py-4">
        <div class="flex flex-wrap items-center gap-3">
            ...
            @php
                $cgLeg=[
                    ['g'=>'A','l'=>'Safest 20%','s'=>'Decile 9–10','fill'=>'#22A620','g1'=>'#15691a','g2'=>'#22A620','glow'=>'rgba(34,166,32,.4)'],
                    ['g'=>'B','l'=>'Below avg',  's'=>'Decile 7–8', 'fill'=>'#399CD8','g1'=>'#1d6fa5','g2'=>'#399CD8','glow'=>'rgba(57,156,216,.4)'],
                    ['g'=>'C','l'=>'Near avg',   's'=>'Decile 5–6', 'fill'=>'#F1B72C','g1'=>'#9a7510','g2'=>'#F1B72C','glow'=>'rgba(241,183,44,.4)'],
                    ['g'=>'D','l'=>'Above avg',  's'=>'Decile 3–4', 'fill'=>'#EE7728','g1'=>'#b4521a','g2'=>'#EE7728','glow'=>'rgba(238,119,40,.4)'],
                    ['g'=>'F','l'=>'Highest crime','s'=>'Decile 1–2','fill'=>'#E6201A','g1'=>'#a8120d','g2'=>'#E6201A','glow'=>'rgba(230,32,26,.4)'],
                ];
            @endphp
            @foreach($cgLeg as $l)
...
<section id="cg-main-section" class="min-h-screen" style="background:#f8fafc;display:none">
    <div class="mx-auto max-w-7xl px-5 sm:px-8 py-10">
        ...
        <div id="cg-states-view">
            ...
            <div id="cg-map-wrap" style="position:relative;background:#fff;border:1px solid rgba(0,0,0,.08);border-radius:16px;margin-bottom:32px;padding:18px 20px 14px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,.06)">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
                    ...
                    <div style="display:flex;align-items:center;gap:10px">
                        @foreach([['A','#22A620'],['B','#399CD8'],['C','#F1B72C'],['D','#EE7728'],['F','#E6201A']] as $mg)
            ...
            <div id="cg-state-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(170px,1fr));gap:16px"></div>
        </div>
...
var GC={A:{bg:'#22A620',tx:'#ffffff'},B:{bg:'#399CD8',tx:'#ffffff'},C:{bg:'#F1B72C',tx:'#3a2c05'},D:{bg:'#EE7728',tx:'#ffffff'},F:{bg:'#E6201A',tx:'#ffffff'}};
...
var LEGEND_COLORS={
  A:{fill:'#22A620',g1:'#15691a',gradient:'linear-gradient(135deg,#15691a,#22A620)',glow:'rgba(34,166,32,.45)'},
  B:{fill:'#399CD8',g1:'#1d6fa5',gradient:'linear-gradient(135deg,#1d6fa5,#399CD8)',glow:'rgba(57,156,216,.45)'},
  C:{fill:'#F1B72C',g1:'#9a7510',gradient:'linear-gradient(135deg,#9a7510,#F1B72C)',glow:'rgba(241,183,44,.45)'},
  D:{fill:'#EE7728',g1:'#b4521a',gradient:'linear-gradient(135deg,#b4521a,#EE7728)',glow:'rgba(238,119,40,.45)'},
  F:{fill:'#E6201A',g1:'#a8120d',gradient:'linear-gradient(135deg,#a8120d,#E6201A)',glow:'rgba(230,32,26,.45)'}
};
var cgLegActive='';
      ...
      .then(function(us){
        ...
        svg.selectAll('path.cg-sp')
          ...
          .attr('fill',function(d){
            ...
            var s=gradeOf[abbr];
            return s?CG_MAP_GRADE[s.grade]:'#1a2744';
          })
          ...
          .on('mouseover',function(event,d){
            ...
            var s=gradeOf[abbr];
            if(s&&tooltip){
              var gc=CG_MAP_GRADE[s.grade]||CG_MAP_GRADE.F;
              tooltip.innerHTML='<div style="font-weight:800;color:'+gc+';font-size:13px">'+s.name+'</div>'
                +'<div style="color:#64748b;font-size:10px;margin-top:2px">Grade <span style="color:'+gc+';font-weight:900">'+s.grade+'</span> · '+s.cities.length+' '+(s.cities.length===1?'city':'cities')+'</div>';
        ...
        svg.append('path')
          ...
          .attr('d',pathGen);
        /* state abbreviation labels (always visible) */
                ...
                <div class="reveal reveal-right lg:col-span-6">
                    <div class="relative mx-auto max-w-xl">
                        ...
                        <div class="relative rounded-[32px] overflow-hidden aspect-[4/5] ring-1 ring-ink-100 lush-shadow agent-slideshow">
                            <img src="/images/hero-agent-slide-k9.jpg" data-mx-src="/images/mx/agent-1.jpg" alt="K-9 officer on duty" class="hero-slide absolute inset-0 w-full h-full object-cover object-center" />
...
<div data-view="citizen-app" class="hidden">
    <style>
        ...
        /* ---- HERO (Fast Response Network composition, LIGHT theme) ---- */
        ...
        [data-view="citizen-app"] .citizen-hero::before {
            ...
            background-image:
                radial-gradient(circle at 30% 30%, rgba(190,30,30,.35) 1px, transparent 1.5px),
                radial-gradient(circle at 70% 70%, rgba(15,23,42,.22) 1px, transparent 1.5px);
                ...
                <div class="relative grid lg:grid-cols-12 gap-10 lg:gap-14 items-center">
                    ...
                    <div class="lg:col-span-7">
                        ...
                        <div class="reveal reveal-delay-3 mt-7 flex flex-wrap items-center gap-3">
                            <a data-route href="#/about" class="inline-flex items-center gap-2 rounded-lg border-2 border-brand-500 text-brand-600 hover:bg-brand-50 px-6 py-2.5 text-sm font-semibold transition">
                                About Auxilio
                            ...
                            <a href="#emergency-types" class="inline-flex items-center gap-2 rounded-lg bg-navy-900 hover:bg-navy-800 text-white px-6 py-2.5 text-sm font-semibold transition shadow-lg">
    ...
    <section id="emergency-types" class="relative overflow-hidden" style="background:linear-gradient(180deg,#fbfaf7 0%, #ffffff 100%);">
        ...
        <div class="relative mx-auto max-w-7xl px-5 sm:px-8 py-20 lg:py-28">
            ...
            <div class="reveal mt-14 rounded-3xl glass-light ring-1 ring-ink-100 p-6 sm:p-8 flex flex-col sm:flex-row items-center gap-5 sm:gap-8 lush-shadow">
                ...
                <a href="#" class="moving-gradient inline-flex items-center gap-2 rounded-lg text-white px-6 py-3 text-sm font-semibold shadow-lg transition hover:-translate-y-0.5">
                    Get Auxilio Free
                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 12h14M13 5l7 7-7 7"/></svg>
                </a>
            </div>
        ...
        <div class="relative mx-auto max-w-7xl px-5 sm:px-8 py-20 lg:py-28">
            <div class="grid lg:grid-cols-12 gap-12 lg:gap-16 items-center">
                ...
                <div class="lg:col-span-6 lg:order-1 fade-up">
                    ...
                    <a href="#" class="mt-8 inline-flex items-center gap-2 moving-gradient text-white rounded-lg px-6 py-3 text-sm font-semibold shadow-2xl transition hover:-translate-y-0.5" style="box-shadow:0 22px 50px -12px rgba(251,6,6,.55)">
                        Try the SOS demo
                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 12h14M13 5l7 7-7 7"/></svg>
                    </a>
                </div>
        ...
        <div class="relative mx-auto max-w-7xl px-5 sm:px-8 py-20 lg:py-32">
            <div class="grid lg:grid-cols-12 gap-12 lg:gap-16 items-center">
                <div class="lg:col-span-7">
                    ...
                    <div class="reveal reveal-delay-3 mt-8 flex flex-wrap items-center gap-3">
                        <a href="#" class="inline-flex items-center gap-2 rounded-lg bg-white text-navy-900 px-6 py-3 text-sm font-bold transition hover:-translate-y-0.5 hover:opacity-95 shadow-lg">
...
{{-- ============================================================
     AUXILIO AI VIEW — intelligent public safety platform
============================================================--}}
<div data-view="auxilio-ai" class="hidden">
    ...
    <section class="relative overflow-hidden py-16 lg:py-20" style="background:linear-gradient(180deg,#050b22,#0a1a4a 50%,#050b22);">
        ...
        <style>
            [data-view="auxilio-ai"] .aux-dash { --aux-red:#E24B4A; --aux-blue:#378ADD; --aux-amber:#e09c3b; --aux-green:#34d399; --aux-purple:#a78bfa; --aux-rose:#fb7185; --aux-olive:#86efac; --aux-deep:#f87171; }
            [data-view="auxilio-ai"] .aux-tab-btn { padding: 8px 18px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; background:rgba(255,255,255,0.07); color:rgba(255,255,255,0.65); border:1px solid rgba(255,255,255,0.12); transition: all .18s ease; display:inline-flex; align-items:center; gap:6px; }
            [data-view="auxilio-ai"] .aux-tab-btn:hover { color:#fff; border-color:rgba(255,255,255,0.25); background:rgba(255,255,255,0.12); }
            ...
            [data-view="auxilio-ai"] .aux-crime-btn { padding: 6px 14px; border-radius: 8px; font-size: 12px; font-weight:500; cursor: pointer; background:rgba(255,255,255,0.07); color:rgba(255,255,255,0.6); border:1px solid rgba(255,255,255,0.12); transition: all .15s ease; }
            [data-view="auxilio-ai"] .aux-crime-btn:hover { color:#fff; background:rgba(255,255,255,0.12); }
            [data-view="auxilio-ai"] .aux-crime-btn.on { color:#fff; border-color:transparent; box-shadow: 0 6px 14px -6px rgba(0,0,0,.4); }
            [data-view="auxilio-ai"] .aux-bar-row { display:flex; align-items:center; gap:10px; margin-bottom:6px; }
            [data-view="auxilio-ai"] .aux-bar-label { font-size:12px; color:rgba(255,255,255,0.45); min-width:78px; text-align:right; font-variant-numeric: tabular-nums; }
            [data-view="auxilio-ai"] .aux-bar-track { flex:1; height:22px; background:rgba(255,255,255,0.08); border-radius:6px; overflow:hidden; }
            ...
            [data-view="auxilio-ai"] .aux-bar-val { font-size:11px; color:rgba(255,255,255,0.45); min-width:36px; font-weight:500; }
            [data-view="auxilio-ai"] .aux-stat-card { background:rgba(255,255,255,0.06); border-radius:14px; padding: 14px 16px; border:1px solid rgba(255,255,255,0.1); }
            [data-view="auxilio-ai"] .aux-stat-num { font-size:22px; font-weight:800; color:#ffffff; font-family: var(--font-display, inherit); }
            [data-view="auxilio-ai"] .aux-stat-lbl { font-size:12px; color:rgba(255,255,255,0.5); margin-top:2px; line-height:1.35; }
            [data-view="auxilio-ai"] .aux-stat-num.red { color:#fb7185; }
            [data-view="auxilio-ai"] .aux-stat-num.amber { color:#fbbf24; }
            [data-view="auxilio-ai"] .aux-stat-num.dn { color:#34d399; }
            [data-view="auxilio-ai"] .aux-legend { display:flex; gap:16px; flex-wrap:wrap; margin-bottom:12px; }
            [data-view="auxilio-ai"] .aux-legend-item { display:flex; align-items:center; gap:6px; font-size:12px; color:rgba(255,255,255,0.5); }
            [data-view="auxilio-ai"] .aux-legend-dot { width:10px; height:10px; border-radius:50%; }
            [data-view="auxilio-ai"] .aux-trend-row { display:flex; align-items:center; gap:10px; padding:9px 0; border-bottom:1px solid rgba(255,255,255,0.08); }
            [data-view="auxilio-ai"] .aux-trend-year { font-size:13px; font-weight:600; color:#fff; min-width:48px; }
            [data-view="auxilio-ai"] .aux-trend-track { flex:1; height:18px; background:rgba(255,255,255,0.08); border-radius:6px; overflow:hidden; }
            ...
            [data-view="auxilio-ai"] .aux-trend-pct { font-size:11px; min-width:54px; text-align:right; font-weight:600; color:rgba(255,255,255,0.7); }
            [data-view="auxilio-ai"] .aux-source { font-size:11px; color:rgba(255,255,255,0.35); margin-top:18px; line-height:1.6; }
            [data-view="auxilio-ai"] .aux-divider { border:none; border-top:1px solid rgba(255,255,255,0.08); margin:20px 0; }
            [data-view="auxilio-ai"] .aux-peak { display:flex; align-items:center; gap:10px; padding:10px 0; border-bottom:1px solid rgba(255,255,255,0.08); flex-wrap:wrap; }
            [data-view="auxilio-ai"] .aux-peak-name { font-size:13px; font-weight:600; color:#fff; min-width:150px; }
            [data-view="auxilio-ai"] .aux-peak-meta { display:flex; gap:14px; flex-wrap:wrap; align-items:center; }
            [data-view="auxilio-ai"] .aux-peak-meta span { font-size:12px; color:rgba(255,255,255,0.5); display:inline-flex; align-items:center; gap:5px; }
            [data-view="auxilio-ai"] .aux-peak-chip { background:rgba(255,255,255,0.1); padding:3px 10px; border-radius:10px; color:rgba(255,255,255,0.7); font-weight:500; }
        </style>
        ...
        <div class="relative mx-auto max-w-6xl px-5 sm:px-8 aux-dash">
            ...
            {{-- Panels --}}
            <div class="ai-anim ai-d2 mt-8 rounded-3xl p-6 sm:p-8" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);backdrop-filter:blur(8px)">
                {{-- Hour --}}
                <div class="aux-panel active" data-aux-panel="hour">
                    <p class="text-xs font-semibold uppercase tracking-[.16em] mb-4" style="color:rgba(255,255,255,0.4)">Crime activity by hour of day</p>
    ...
    <section class="relative py-16 lg:py-20 overflow-hidden text-white" style="background:linear-gradient(160deg, #050d2a 0%, #0a1a4a 50%, #061229 100%);">
        ...
        <span class="pointer-events-none absolute -bottom-20 -right-10 w-[460px] h-[460px] rounded-full blur-3xl" style="background:radial-gradient(circle,rgba(99,102,241,.22) 0%,transparent 70%);"></span>

        <div class="relative mx-auto max-w-5xl px-5 sm:px-8">
            <div class="text-center max-w-2xl mx-auto">
                <p class="text-xs font-semibold uppercase tracking-[.2em]" style="color:#7dd3fc;">Taglines</p>
                <h2 class="mt-3 font-display font-extrabold text-3xl sm:text-4xl tracking-tight">
                    How we <span style="background:linear-gradient(90deg,#38bdf8,#a5b4fc); -webkit-background-clip:text; background-clip:text; color:transparent;">say it.</span>
                </h2>
            </div>
            @php
                $taglines = [
                    'Your Safety. Your Control.',
                    'AI Protection in Real Time.',
                    'Smarter Safety for Every Family.',
                    'The Intelligent Safety Network.',
                    'When Seconds Matter, Auxilio Responds.',
                    'Connected Protection Powered by AI.',
                    'Real-Time Safety Starts Here.',
                    'The Future of Community Protection.',
                ];
            @endphp
            <div class="mt-12 grid sm:grid-cols-2 gap-3">
                @foreach ($taglines as $tag)
                    <div class="rounded-2xl px-5 py-4 flex items-center gap-3" style="background:rgba(255,255,255,.04); box-shadow: inset 0 0 0 1px rgba(255,255,255,.08);">
                        <span class="grid place-items-center shrink-0 w-8 h-8 rounded-full text-navy-900 font-bold" style="background:linear-gradient(135deg,#38bdf8,#a5b4fc);">"</span>
                        <p class="font-display font-semibold text-white">{{ $tag }}</p>
                    </div>
                @endforeach
            </div>

            <div class="mt-12 text-center">
                <a href="#" data-open-chat class="inline-flex items-center gap-2 rounded-lg px-6 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5" style="background:linear-gradient(135deg,#0ea5e9,#6366f1); box-shadow: 0 14px 30px -10px rgba(14,165,233,.55);">
                    Try Auxilio AI now
                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4"><path stroke-linecap="round" stroke-linejoin="round" d="M5 12h14m0 0l-6-6m6 6l-6 6"/></svg>
                </a>
            </div>
...
[data-view="er"] .ea-grid-bg {
    ...
    background-image:
        ...
        linear-gradient(90deg, rgba(120,53,15,.07) 1px, transparent 1px);
    background-size: 56px 56px;
...
[data-view="er"] .ea-light .ea-card {
    ...
    box-shadow:
        ...
        0 12px 32px -18px rgba(15,23,42,.22);
    isolation: isolate;
}
...
[data-view="er"] *:not(.ea-pulse-dot):not(.ea-circle-badge):not(.ea-avatar):not(.ea-radio):not(.ea-status-pill),
[data-view="er"] *::before,
...
function eaApp() {
    return {
        ...
        typeTag(t) {
            const map = {
                'shots-fired':      {tag:'SF',   color:'#E6201A'},
                'armed-robbery':    {tag:'AR',   color:'#EE7728'},
                'structure-fire':   {tag:'FIRE', color:'#E6201A'},
                'assault':          {tag:'AS',   color:'#F1B72C'},
                'vehicle-accident': {tag:'VA',   color:'#399CD8'},
                'carjacking':       {tag:'CJ',   color:'#EE7728'},
                'break-in':         {tag:'BI',   color:'#EE7728'},
                'domestic':         {tag:'DV',   color:'#F1B72C'},
                'medical':          {tag:'MED',  color:'#22A620'},
            };
            return map[t] || {tag:'INC', color:'#22A620'};
        },
        agentAvatars(inc) {
            const pool = [
...
<div x-data="eaApp()" x-init="init()" x-cloak>
    ...
    {{-- ─── LIVE TICKER ─── fixed at bottom of viewport (only renders inside ER view) --}}
    <div class="overflow-hidden" style="position:fixed;left:0;right:0;bottom:0;z-index:40;background:#0a0f1a;border-top:1px solid rgba(220,38,38,.3);height:34px;">
    ...
    {{-- ─── HERO ─── --}}
    <section class="ea-hero-bg relative overflow-hidden" style="min-height:90vh;display:flex;align-items:center;">
        ...
        <div class="hidden md:block" style="position:absolute;right:10%;top:50%;transform:translateY(-50%);width:clamp(260px,18vw,330px);height:clamp(260px,18vw,330px);pointer-events:none;z-index:0;">
        ...
        <div class="relative z-10 w-full mx-auto max-w-7xl px-5 sm:px-8" style="padding-top:2rem;padding-bottom:3rem;">
            <div>
                <div>
                    ...
                    <div class="flex items-center gap-5 mb-5">
                        <div>
                            <div class="flex items-baseline gap-2">
                                <span class="font-mono font-black leading-none" style="font-size:clamp(36px,4vw,56px);color:#0a1a4a" x-text="liveCount">0</span>
                                <span class="ea-pulse-dot w-2 h-2 rounded-full bg-red-600 self-center"></span>
                            </div>
                            <p class="text-[10px] font-bold uppercase tracking-[.2em] text-red-700 mt-1">Active Incidents Now</p>
                        </div>
                        <div class="h-10 w-px" style="background:rgba(10,26,74,.22)"></div>
                        <div>
                            <div class="font-mono text-xl font-bold" style="color:#0a1a4a" x-text="liveTime">--:--:--</div>
                            <p class="text-[10px] font-semibold uppercase tracking-[.18em] mt-1" style="color:rgba(10,26,74,.55)">System Time · EST</p>
                        </div>
                    </div>

                    <div class="inline-flex items-center gap-2.5 rounded-full px-3.5 py-1.5 mb-5" style="background:rgba(255,255,255,.6);border:1px solid rgba(120,53,15,.22);">
                    ...
                    <h1 class="ea-hero-title font-black text-5xl lg:text-[64px] leading-[1.02] tracking-tight mb-4" style="color:#0a1a4a">
                        Real-time<br>
                    ...
                    <p class="text-base lg:text-lg max-w-md leading-relaxed mb-5" style="color:rgba(10,26,74,.72)">
    ...
    <section class="mx-auto max-w-7xl px-5 sm:px-8 mt-10">
        <div class="flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between">
            ...
            <div class="flex items-center gap-1.5 overflow-x-auto" style="scrollbar-width:none;">
                <template x-for="sev in [
                    {k:'all',     l:'All',      c:'#0f172a'},
                    {k:'critical',l:'Critical', c:'#ef4444'},
                    {k:'high',    l:'High',     c:'#f97316'},
                    {k:'active',  l:'Active',   c:'#3b82f6'},
                    {k:'resolved',l:'Resolved', c:'#10b981'}
                ]" :key="sev.k">
                    <button @click="activeSeverity = sev.k" class="ea-pill shrink-0 px-3.5 py-1.5 rounded-md text-[12px] font-semibold inline-flex items-center gap-1.5"
        ...
        <div class="mt-3 flex items-center gap-1.5 overflow-x-auto" style="scrollbar-width:none;">
            ...
            <template x-for="t in [
                ...
                <button @click="activeFilter = t.k" class="ea-pill shrink-0 px-3 py-1 rounded-md text-[11px] font-semibold"
    ...
    <section x-show="!loading" class="mx-auto max-w-7xl px-5 sm:px-8 mb-16">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <template x-for="(inc, idx) in filtered" :key="inc.id">
                <div class="ea-card ea-card-anim rounded-2xl relative overflow-hidden bg-white border border-slate-200"
                    ...
                    <div class="p-5">
                        ...
                        <div class="flex items-center gap-2.5 mb-3">
                            <span class="ea-radio inline-flex w-5 h-5 shrink-0 items-center justify-center" style="border:2px solid #0a1a4a; border-radius:9999px;">
                                <span class="ea-radio w-2 h-2" style="background:#0a1a4a; border-radius:9999px;"></span>
                            </span>
                        ...
                        <div class="grid grid-cols-3 gap-2 mb-4">
                            <div class="rounded-lg py-2 text-center" style="border:1px dashed #cbd5e1;">
                        ...
                        <div class="flex items-center justify-between gap-2">
                            <div class="flex items-center -space-x-2">
                                <img src="/images/officer-1.jpg" alt="" class="ea-avatar w-8 h-8 ring-2 ring-white object-cover" style="border-radius:9999px;" />
                                <img src="/images/officer-2.jpg" alt="" class="ea-avatar w-8 h-8 ring-2 ring-white object-cover" style="border-radius:9999px;" />
                                <img src="/images/officer-3.jpg" alt="" class="ea-avatar w-8 h-8 ring-2 ring-white object-cover" style="border-radius:9999px;" />
...
<style>
    ...
    #chat-dialog.is-wide { width: min(640px, calc(100vw - 2rem)); }
    .chat-bubble { max-width: 80%; padding:10px 14px; font-size:13.5px; line-height:1.45; }
    .bubble-me { background:#dbeafe; color:#0c1126; border-radius:14px 14px 4px 14px; }
    .bubble-them { background:#f3f4f7; color:#0c1126; border-radius:14px 14px 14px 4px; }
    .typing-pill { display:inline-flex; align-items:center; gap:3px; padding:9px 11px; background:#f3f4f7; border-radius:14px; }
    .typing-dot { display:inline-block; width:6px; height:6px; border-radius:9999px; background:#9aa3b2; animation: typing 1.2s infinite; }
    ...
    .chat-toolbar-btn { display:inline-flex; align-items:center; gap:6px; padding:8px 10px; font-size:13px; color:#1e3a8a; font-weight:500; border-radius:6px; transition:background .15s; }
    .chat-toolbar-btn:hover { background:#eef2ff; }
</style>
...
<div id="chat-dialog" role="dialog" aria-modal="false" aria-labelledby="chat-title"
    ...
    <header class="flex items-center justify-between px-4 py-3" style="background:#1e3a8a;">
        <button id="chat-expand" type="button" class="grid place-items-center w-7 h-7 rounded-md text-white hover:bg-white/15" aria-label="Expand chat">
            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7H4v4M16 17h4v-4M4 7l6 6M20 17l-6-6"/></svg>
        </button>
        <p id="chat-title" class="sr-only">Auxilio AI Assistant</p>
        <button id="chat-close" type="button" class="grid place-items-center w-7 h-7 rounded-md text-white hover:bg-white/15" aria-label="Close chat">
            <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4"><path stroke-linecap="round" stroke-linejoin="round" d="M6 9l6 6 6-6"/></svg>
        </button>
    </header>

    {{-- White toolbar: Contact / Chat history / New chat — tight against header --}}
    <div class="flex items-center justify-around gap-1 px-2 py-2.5 bg-white border-b border-ink-100">
        <button type="button" class="chat-toolbar-btn">
        ...
        <button id="chat-new" type="button" class="chat-toolbar-btn">
            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 5v14M5 12h14"/></svg>
            New chat
        </button>
    </div>
    ...
    {{-- Composer — rounded grey input + navy paper plane on the right --}}
    <form id="chat-form" class="px-3 py-3 bg-white border-t border-ink-100">
        <div class="relative">
            <input id="chat-input" type="text" autocomplete="off" placeholder="Ask me anything..." class="w-full pl-4 py-3 text-sm rounded-full bg-ink-50 border border-ink-100 focus:outline-none focus:border-blue-400 focus:bg-white focus:ring-2 focus:ring-blue-100" style="padding-right:54px;"/>
            <button type="submit" class="grid place-items-center w-9 h-9 rounded-full text-white" style="position:absolute; top:50%; right:6px; transform:translateY(-50%); background:#1e3a8a;" aria-label="Send">
...
(function(){
    ...
    var sparkleSvg = '<svg class="w-4 h-4" style="color:#1e3a8a;" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l1.5 5 5 1.5-5 1.5L12 15l-1.5-5-5-1.5 5-1.5L12 2zm7 11l.9 2.8 2.8.9-2.8.9-.9 2.8-.9-2.8-2.8-.9 2.8-.9.9-2.8zM5 14l.6 1.9 1.9.6-1.9.6-.6 1.9-.6-1.9-1.9-.6 1.9-.6.6-1.9z"/></svg>';

    function appendBubble(from, text){
        if (emptyEl && !emptyEl.classList.contains('hidden')) emptyEl.classList.add('hidden');
        var html;
        if (from === 'me') {
            html = '<div class="flex justify-end"><div class="chat-bubble bubble-me">'+escapeHtml(text)+'</div></div>';
        } else {
            html = '<div class="space-y-1.5">'
                +'<div class="flex items-center gap-1.5">'+sparkleSvg+'<span class="text-[13px] font-semibold text-navy-900">AI Assistant</span></div>'
                +'<div class="chat-bubble bubble-them">'+escapeHtml(text)+'</div>'
                +'</div>';
        }
    ...
    function showTyping(){
        ...
        threadEl.insertAdjacentHTML('beforeend',
            '<div id="'+id+'" class="space-y-1.5">'
            +'<div class="flex items-center gap-1.5">'+sparkleSvg+'<span class="text-[13px] font-semibold text-navy-900">AI Assistant</span></div>'
            +'<div class="typing-pill"><span class="typing-dot"></span><span class="typing-dot"></span><span class="typing-dot"></span></div>'
            +'</div>'
        );
        threadEl.scrollTop = threadEl.scrollHeight;
        return id;
    ...
    if (expandBtn) {
        expandBtn.addEventListener('click', function(){
            dialog.classList.toggle('is-wide');
        });
    }

    // New chat — clear thread and restore empty state
